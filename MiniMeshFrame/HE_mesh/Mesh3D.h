...
public:
	...
	BoundaryTag boundary_flag_;	//!< boundary flag
	HE_vert		*start_;		//!< vertex at the end of the half-edge
	//HE_edge		*ppair_;		//!< oppositely oriented adjacent half-edge
	 int subgraphflag;
public:
	HE_edge()
...
public:
	...
	int Computecutchoice(int i);

	bool computeSubgraph(int flag);

	void ComputeSumVector(int id, int flag);
	void ComputeAngle(int id, Vec3f sum);
	bool findsubgraph(int * roots_, int num_roots_, int * cuts, int num_cuts_);

	void DFS(HE_vert* last_, HE_vert* cur, int flag);

	//void Reset(int numRoots);
...
/*----------------------------------add by wang kang at 2013-10-12- -----------------------------------*/
	void get_neighborId(const int& vertid, std::vector<int>& neighbors)
	{
