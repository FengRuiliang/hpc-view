...
void RenderingWidget::compute()
{
	ptr_mesh_->computeRoot();
}
...
void RenderingWidget::DrawEdge(bool bv)
{
	...
	if (ptr_mesh_->num_of_edge_list() == 0)
	{
		return;
	}
	...
	{ 
		HE_edge *pedge(edges.at(i));
		//glClearColor(0.0,0.0,0.0,0.0);
		switch (pedge->subgraphflag)
		{
		case 1:glColor3f(1.0, 0.0, 0.0);
			glBegin(GL_LINE_LOOP);
			glVertex3fv(pedge->pvert_->position().data());
			glVertex3fv(pedge->start_->position().data());
			glEnd();
